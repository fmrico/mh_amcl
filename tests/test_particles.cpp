...
public:
  ...
  tf2::Transform get_tranform_to_read_vector_test(
    const sensor_msgs::LaserScan & scan, int index, double module = 1.0)
  {
    return get_tranform_to_read_vector(scan, index, module);
  }
  ...
  {
    return get_distance_to_obstacle(map2bf, bf2laser, uvector, scan, costmap);
  }
};
...
TEST(test1, test_init)
{
  ...
  ASSERT_NEAR(mean_x, 0.0, 0.01);
  ASSERT_NEAR(stdev_x, 0.1, 0.05);
  ASSERT_NEAR(mean_y, 0.0, 0.01);
...
TEST(test1, test_get_get_tranform_to_read_vector)
{
  ParticlesDistributionTest particle_dist;
  ...
  {
    tf2::Transform point_0 = particle_dist.get_tranform_to_read_vector_test(scan, 0);
    tf2::Transform point_1 = particle_dist.get_tranform_to_read_vector_test(scan, 1);
    tf2::Transform point_2 = particle_dist.get_tranform_to_read_vector_test(scan, 2);
    tf2::Transform point_3 = particle_dist.get_tranform_to_read_vector_test(scan, 3);
    tf2::Transform point_4 = particle_dist.get_tranform_to_read_vector_test(scan, 4);
    ...
    ASSERT_NEAR(point_1.getOrigin().y(), -1.0, 0.0001);
    ASSERT_NEAR(point_1.getOrigin().z(), 0.0, 0.0001);

    ASSERT_NEAR(point_2.getOrigin().x(), 1.0, 0.0001);
    ...
    ASSERT_NEAR(point_3.getOrigin().y(), 1.0, 0.0001);
    ASSERT_NEAR(point_3.getOrigin().z(), 0.0, 0.0001);

    ASSERT_NEAR(point_4.getOrigin().x(), -1.0, 0.0001);
    ASSERT_NEAR(point_4.getOrigin().y(), 0.0, 0.0001);
    ASSERT_NEAR(point_4.getOrigin().z(), 0.0, 0.0001);
  }

  {
    tf2::Transform point_0 = particle_dist.get_tranform_to_read_vector_test(scan, 0, 0.01);
    tf2::Transform point_1 = particle_dist.get_tranform_to_read_vector_test(scan, 1, 0.01);
    tf2::Transform point_2 = particle_dist.get_tranform_to_read_vector_test(scan, 2, 0.01);
    tf2::Transform point_3 = particle_dist.get_tranform_to_read_vector_test(scan, 3, 0.01);
    tf2::Transform point_4 = particle_dist.get_tranform_to_read_vector_test(scan, 4, 0.01);

    ASSERT_NEAR(point_0.getOrigin().x(), -0.01, 0.0001);
    ASSERT_NEAR(point_0.getOrigin().y(), 0.0, 0.0001);
    ASSERT_NEAR(point_0.getOrigin().z(), 0.0, 0.0001);

    ASSERT_NEAR(point_1.getOrigin().x(), 0.0, 0.0001);
    ASSERT_NEAR(point_1.getOrigin().y(), -0.01, 0.0001);
    ASSERT_NEAR(point_1.getOrigin().z(), 0.0, 0.0001);

    ASSERT_NEAR(point_2.getOrigin().x(), 0.01, 0.0001);
    ASSERT_NEAR(point_2.getOrigin().y(), 0.0, 0.0001);
    ASSERT_NEAR(point_2.getOrigin().z(), 0.0, 0.0001);

    ASSERT_NEAR(point_3.getOrigin().x(), 0.0, 0.0001);
    ASSERT_NEAR(point_3.getOrigin().y(), 0.01, 0.0001);
    ASSERT_NEAR(point_3.getOrigin().z(), 0.0, 0.0001);

    ASSERT_NEAR(point_4.getOrigin().x(), -0.01, 0.0001);
    ASSERT_NEAR(point_4.getOrigin().y(), 0.0, 0.0001);
    ASSERT_NEAR(point_4.getOrigin().z(), 0.0, 0.0001);
  }
}
...
TEST(test1, get_distance_to_obstacle)
{
  ...
  map2bf.setRotation({0.0, 0.0, 0.0, 1.0});
  ...
  // Real tests
  {
    ...
    bool is_inf = std::isinf(scan.ranges[0]);
    if (!is_inf) laser2point = particle_dist.get_tranform_to_read_test(scan, 0);
    tf2::Transform laser2point_uvectortf = particle_dist.get_tranform_to_read_vector_test(scan, 0);

    double distance = particle_dist.get_distance_to_obstacle_test(map2bf, bf2lasert,
      laser2point_uvectortf, scan, costmap);
  ...
  {
    tf2::Transform laser2point;
    bool is_inf = std::isinf(scan.ranges[1]);
    if (!is_inf) laser2point = particle_dist.get_tranform_to_read_test(scan, 1);
    tf2::Transform laser2point_uvectortf = particle_dist.get_tranform_to_read_vector_test(scan, 1);
  ...
  {
    tf2::Transform laser2point;
    bool is_inf = std::isinf(scan.ranges[2]);
    if (!is_inf) laser2point = particle_dist.get_tranform_to_read_test(scan, 2);
    tf2::Transform laser2point_uvectortf = particle_dist.get_tranform_to_read_vector_test(scan, 2);
  ...
  {
    tf2::Transform laser2point;
    bool is_inf = std::isinf(scan.ranges[3]);
    if (!is_inf) laser2point = particle_dist.get_tranform_to_read_test(scan, 3);
    tf2::Transform laser2point_uvectortf = particle_dist.get_tranform_to_read_vector_test(scan, 3);
  ...
  {
    tf2::Transform laser2point;
    bool is_inf = std::isinf(scan.ranges[4]);
    if (!is_inf) laser2point = particle_dist.get_tranform_to_read_test(scan, 4);
    tf2::Transform laser2point_uvectortf = particle_dist.get_tranform_to_read_vector_test(scan, 4);
...
TEST(test1, test_correct)
{
  ...
  unsigned int mx , my;
  costmap.worldToMap(1.0, 0.0, mx, my);
  costmap.setCost(mx, my, costmap_2d::LETHAL_OBSTACLE);
  ...
  {
    if (fabs(particle.pose.getOrigin().x()) < 0.05 && fabs(particle.pose.getOrigin().y()) < 0.05)
    {
      ASSERT_GT(particle.prob, 1.0 / particles.size());
      std::cerr << "[(" << particle.pose.getOrigin().x() << ", " << particle.pose.getOrigin().y() << ") > " << particle.prob << "] ";
    }
    else
    {
      ASSERT_LT(particle.prob, 1.0 / particles.size());
      std::cerr << "[(" << particle.pose.getOrigin().x() << ", " << particle.pose.getOrigin().y() << ") < " << particle.prob << "] ";
    }
  }
